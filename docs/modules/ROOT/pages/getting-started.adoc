...
Les services déployés dans Minikube ne sont pas directement accessibles via des URLs fixes sur localhost. Vous devez utiliser la commande `minikube service` pour créer un tunnel d'accès.

==== Accéder au Service de Sélection

[source,bash]
----
# Cette commande ouvre automatiquement un navigateur avec l'URL correcte
...
[IMPORTANT]
====
Sur Windows avec le driver Docker pour Minikube, la fenêtre du terminal doit rester ouverte pendant que vous accédez au service, car elle maintient le tunnel actif.
====
