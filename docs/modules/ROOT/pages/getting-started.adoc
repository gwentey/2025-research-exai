...
. *Construire les images Docker pour Minikube :*
+
Avant d'exécuter le script de déploiement, assurez-vous que les images Docker sont disponibles dans l'environnement Minikube :
+
[source,bash]
----
...
# Construire l'image Docker pour l'API Gateway
cd api-gateway
docker build -t exai-api-gateway:latest .
cd ..

# Construire l'image Docker pour le Service Selection
cd service-selection
docker build -t service-selection:latest .
cd ..
----
+
[IMPORTANT]
====
Si vous rencontrez des erreurs de démarrage des pods `api-gateway` ou `service-selection` avec des messages comme `ImagePullBackOff`, assurez-vous que les images Docker sont bien construites dans l'environnement Minikube et que les fichiers de déploiement `deployment.yaml` utilisent `imagePullPolicy: Never` pour utiliser uniquement les images locales.
====
...
. *Résolution de problèmes courants après déploiement :*
+
--
* Si le pod `api-gateway` est en état d'erreur avec un message `CrashLoopBackOff` ou si les logs montrent une erreur `ModuleNotFoundError: No module named 'app'`, vous devez reconstruire l'image Docker avec la structure de répertoire corrigée. Utilisez le Dockerfile suivant :
+
[source,dockerfile]
----
...
----
+
Ensuite, reconstruisez l'image et supprimez le pod défaillant pour qu'il soit redémarré :
+
[source,bash]
----
...
# Supprimer le pod défaillant (remplacez XXXXX par l'identifiant réel du pod)
kubectl delete pod -n exai api-gateway-XXXXX
----

* Si le pod `service-selection` est en état d'erreur avec `ImagePullBackOff` ou `ErrImagePull`, vérifiez que l'image est correctement construite dans Minikube et que le fichier deployment.yaml utilise `imagePullPolicy: Never`.
--
...
Une fois que tous les pods sont `Running`, vous pouvez accéder à l'application via les services correspondants.
...
----
+
Cette commande créera un tunnel et ouvrira le service dans votre navigateur par défaut. Vous verrez une URL comme `http://127.0.0.1:XXXXX` dans le terminal.
...
----
+
Cette commande créera un tunnel et ouvrira le service dans votre navigateur par défaut. Vous verrez une URL comme `http://127.0.0.1:YYYYY` dans le terminal.

. *Vérifier l'état des services :*
+
Pour voir la liste complète des services disponibles et leurs URLs :
+
[source,bash]
----
minikube service list -n exai
----

. Accédez à la documentation interactive (Swagger UI) en ajoutant `/docs` à l'URL obtenue : `http://127.0.0.1:XXXXX/docs`.
...
Cette section présente les solutions aux problèmes les plus courants que vous pourriez rencontrer lors du déploiement d'EXAI.
...
*   **Pod bloqué en `ImagePullBackOff` ou `ErrImagePull`:** 
    ** L'image Docker n'a pas pu être trouvée ou téléchargée.
    ** Assurez-vous d'avoir configuré l'environnement Docker pour utiliser celui de Minikube avec `eval $(minikube -p minikube docker-env)` (Linux/macOS) ou `& minikube -p minikube docker-env --shell powershell | Invoke-Expression` (Windows PowerShell) *avant* de lancer `docker build`. 
    ** Vérifiez que le nom et le tag de l'image dans `deployment.yaml` correspondent exactement à ceux utilisés lors du `docker build`.
    ** Modifiez l'attribut `imagePullPolicy` à `Never` dans le fichier `deployment.yaml` pour forcer Kubernetes à utiliser uniquement les images locales.
...
*   **Pod en état `CrashLoopBackOff` ou erreur d'exécution :**
    ...
    ** Pour les erreurs de type `ModuleNotFoundError: No module named 'app'` dans l'API Gateway, référez-vous à la section 3.3.5 pour corriger le Dockerfile et reconstruire l'image.

=== Problèmes liés aux services

*   **Service inaccessible :** 
    ** Vérifiez que le service a été créé avec `kubectl get services -n exai`
    ** Assurez-vous que le pod associé est en état `Running` avec `kubectl get pods -n exai`
    ** Utilisez `minikube service <nom-du-service> -n exai` pour créer un tunnel et accéder au service

*   **`minikube service ... --url` ne fonctionne pas :** 
    ** Assurez-vous que le service Kubernetes existe (`kubectl get services -n exai`). 
    ** Vérifiez l'état du pod associé.
    ** Sur Windows, parfois il est nécessaire de garder le terminal ouvert pendant l'utilisation du tunnel.

=== Comment diagnostiquer un problème

Pour diagnostiquer un problème non listé ci-dessus, suivez ces étapes :

1. Vérifiez l'état des pods : `kubectl get pods -n exai`
2. Pour un pod problématique, consultez les détails : `kubectl describe pod <nom-du-pod> -n exai`
3. Examinez les logs du pod : `kubectl logs -n exai <nom-du-pod>`
4. Vérifiez les services : `kubectl get services -n exai`
