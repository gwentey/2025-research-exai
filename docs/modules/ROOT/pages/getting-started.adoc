...
2.  **Configurer la connexion locale :** S'assurer que les fichiers `.env` à la racine des répertoires `api-gateway` et `service-selection` contiennent la bonne `DATABASE_URL` pointant vers `localhost:5432` (voir le guide détaillé).
